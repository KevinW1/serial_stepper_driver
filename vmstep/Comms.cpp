...
void SerialTransciever::send(byte reply_code, const byte msg[], size_t length) {
...
void SerialTransciever::send(byte reply_code, String msg) {
    ...
    send(reply_code, reinterpret_cast<const byte *>(msg.c_str()), msg.length());
}


void SerialTransciever::recieve() {
    int incoming_byte = -1;
    while ((receiver_state != ReceiverState::MSG_PENDING) && (incoming_byte = Serial.read()) != -1) {
